...
var validate = {
  required: /* @__PURE__ */ __name((value, fieldName) => {
    if (value === void 0 || value === null || String(value).trim() === "") {
      throw new ValidationError(`${fieldName} is required`);
    }
  }, "required"),
  maxLength: /* @__PURE__ */ __name((value, max, fieldName) => {
    if (value && String(value).length > max) {
      throw new ValidationError(`${fieldName} must be less than ${max} characters`);
    }
  }, "maxLength"),
  isValidSlug: /* @__PURE__ */ __name((value) => {
    if (!value) return false;
    const slugRegex = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
    return slugRegex.test(value);
  }, "isValidSlug"),
  isAdult: /* @__PURE__ */ __name((dateOfBirthISO) => {
  ...
  isValidUKMobile: /* @__PURE__ */ __name((value) => {
    if (!value) return false;
...
adminProductsRouter.put("/:id", async (c) => {
  try {
    ...
    const normalizedCondition = sanitizeCondition(body.condition);
    if (body.category_id !== void 0) addUpdate("category_id", body.category_id);
    if (body.name !== void 0) addUpdate("name", body.name);
    addUpdate("slug", slug);
    if (body.description !== void 0) addUpdate("description", body.description);
    if (body.condition !== void 0 || body.refurbished_details !== void 0) {
      addUpdate("condition", normalizedCondition);
    }
    if (body.refurbished_details !== void 0) {
    ...
    if (body.is_featured !== void 0) addUpdate("is_featured", body.is_featured ? 1 : 0);
    if (body.is_active !== void 0) addUpdate("is_active", body.is_active ? 1 : 0);
...
adminDealsRouter.put("/:id", async (c) => {
  try {
    ...
    if (body.product_id !== void 0) addUpdate("product_id", body.product_id);
    ...
    if (body.upfront_cost !== void 0) addUpdate("upfront_cost", body.upfront_cost);
    if (body.data_gb !== void 0) addUpdate("data_gb", body.data_gb);
    if (body.minutes !== void 0) addUpdate("minutes", body.minutes);
    if (body.texts !== void 0) addUpdate("texts", body.texts);
    ...
    if (body.is_active !== void 0) addUpdate("is_active", body.is_active ? 1 : 0);
    if (body.sort_order !== void 0) addUpdate("sort_order", body.sort_order);
...
var DEFAULT_THEME_COLOR = '{"accent":"#2563eb","heroBackground":"radial-gradient(circle at 70% 18%, #b7cdf655 0, transparent 32%), linear-gradient(135deg, #10214f 0%, #020617 58%, #2563eb 145%)","heroGlow":"radial-gradient(circle, #2563eb80 0%, #2563eb28 38%, transparent 72%)","heroPanel":"linear-gradient(135deg, #2563eb 0%, #10214f 100%)"}';
...
adminCategoriesRouter.post("/", async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, 1, ?)
      RETURNING *
    `).bind(body.name, body.slug, displayOrder, themeColor).first();
    return c.json({ success: true, data: result }, 201);
  } catch (err) {
    if (err instanceof ValidationError) return c.json({ success: false, error: err.message }, 400);
...
// src/utils/email.ts
var SENDER_EMAIL = "orders@mobello.uk";
var SENDER_NAME = "Mobello.UK";
var SUPPORT_EMAIL = "support@mobello.uk";
var SUPPORT_PHONE = "0800 123 4567";
async function callResendApi(apiKey, to, subject, html) {
  try {
...
__name(callResendApi, "callResendApi");
          ...
          <tr>
            <td style="background-color:#f9fafb;padding:24px 40px;border-top:1px solid #e5e7eb;
              ...
              <p style="margin:0;color:#9ca3af;font-size:12px;">
                \xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} Mobello.UK. All rights reserved.
              </p>
            </td>
          </tr>
        </table>
...
__name(wrapInLayout, "wrapInLayout");
function getEstimatedDelivery(customDate) {
...
__name(getEstimatedDelivery, "getEstimatedDelivery");
  ...
  const html = wrapInLayout(`
    ...
    <div style="background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%);
      ...
      <p style="margin:0 0 4px;color:#1d4ed8;font-size:12px;font-weight:600;
                text-transform:uppercase;letter-spacing:1px;">\u{1F4E6} Estimated Delivery</p>
...
function buildShippingEmail(data, status) {
...
__name(buildShippingEmail, "buildShippingEmail");
async function sendOrderEmail(type, env2, data) {
  ...
  const result = await callResendApi(env2.RESEND_API_KEY, data.customerEmail, subject, html);
  if (!result.success) {
...
function buildPreorderConfirmationEmail(data) {
  ...
  const html = wrapInLayout(`
    ...
    <div style="background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%);
      ...
      <p style="margin:0 0 12px;color:#1e3a8a;font-size:13px;font-weight:700;
                text-transform:uppercase;letter-spacing:0.5px;">\u{1F4C5} Next Steps & Key Dates</p>
      <ul style="margin:0;padding-left:20px;color:#334155;font-size:14px;line-height:1.7;">
        ...
        <li><strong>Change of mind?</strong> Your \xA3${data.depositAmount.toFixed(2)} deposit is 100% refundable at any time prior to dispatch.</li>
      </ul>
    </div>
...
__name(buildPreorderConfirmationEmail, "buildPreorderConfirmationEmail");
async function sendPreorderEmail(env2, data) {
  if (!env2.RESEND_API_KEY) {
...
__name(sendPreorderEmail, "sendPreorderEmail");
async function sendAdminPreorderNotification(env2, data) {
  if (!env2.RESEND_API_KEY) {
    console.warn("[email] RESEND_API_KEY not set \u2014 skipping admin preorder notification");
    return { success: false, error: "RESEND_API_KEY not configured" };
  }
  const subject = `\u{1F525} New iPhone 18 Pre-Order \u2014 ${data.reservationRef} (${data.model})`;
  const html = wrapInLayout(`
    <div style="text-align:center;margin-bottom:28px;">
      <div style="display:inline-block;background-color:#dbeafe;color:#1e40af;
        ...
        \u{1F525} NEW PRE-BOOKING RESERVATION
      </div>
    </div>
    ...
    <p style="margin:0 0 24px;color:#4b5563;font-size:15px;">
      ...
      <strong>Deposit:</strong> \xA3${data.depositAmount.toFixed(2)} (Paid)
    </p>
  `);
  const result = await callResendApi(env2.RESEND_API_KEY, ADMIN_NOTIFICATION_EMAIL, subject, html);
  return result;
}
__name(sendAdminPreorderNotification, "sendAdminPreorderNotification");
...
checkoutRouter.post("/create-intent", async (c) => {
  try {
    ...
    if (!body.sameAsDelivery) {
      ...
      if (!sanitise.string(billing2.line1)) errors["billing.line1"] = "Billing address line 1 is required";
      if (!sanitise.string(billing2.city)) errors["billing.city"] = "Billing city is required";
      if (!sanitise.string(billing2.postcode)) errors["billing.postcode"] = "Billing postcode is required";
    }
    ...
    console.error("Checkout error:", error3?.message ?? error3);
    if (error3?.stack) console.error("Stack:", error3.stack);
    return c.json({
      success: false,
      message: "Internal Server Error",
      // Surface the real error cause in logs (Cloudflare dashboard)
      debug: error3?.message ?? String(error3)
    }, 500);
  }
});
...
var ordersRouter = new Hono2();
var VALID_STATUSES = ["PENDING", "CONFIRMED", "SHIPPED", "OUT_FOR_DELIVERY", "DELIVERED", "CANCELLED"];
...
ordersRouter.get("/", async (c) => {
  try {
    ...
    const totalRow = await c.env.DB.prepare("SELECT COUNT(*) AS total FROM orders").first();
    return c.json({
      success: true,
      data: rows.results,
...
__name(generateReservationRef, "generateReservationRef");
...
preorderRouter.post("/reserve", async (c) => {
  try {
    ...
    const purchaseType = body.purchaseType === "contract" ? "contract" : "outright";
    ...
    const depositAmount = product.deposit_amount || 99;
...
var preorder_default = preorderRouter;
...
api.route("/preorder", preorder_default);
...
adminRoutes.route("/orders", orders_default);
...
// .wrangler/tmp/bundle-qvTwzS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qvTwzS/middleware-loader.entry.ts
