...
function ukMobileValidator(control: AbstractControl): ValidationErrors | null {
  if (!control.value) return null;
  const stripped = String(control.value).replace(/[\s-]/g, '');
  const regex = /^(?:0|\+44)7\d{9}$/;
  return regex.test(stripped) ? null : { invalidUkMobile: true };
}
...
export class Iphone18PreorderComponent implements OnInit, OnDestroy {
  ...
  private initForm(): void {
    this.reservationForm = this.fb.group({
      ...
      customerPhone: ['', [Validators.required, ukMobileValidator]],
  ...
  // Gallery angle selection (1: Front, 2: 45 deg, 3: Side profile)
  ...
  submitReservation(): void {
    if (this.reservationForm.invalid) {
      ...
      this.toast.error('Please fill in your name, valid email, and UK mobile number.');
      return;
    }
    ...
    const req = {
      ...
      marketingOptIn: fv.marketingOptIn
    };
    ...
    this.preorder.createReservation(req).subscribe({
      ...
      error: (err) => {
        this.isSubmitting.set(false);
        const errMsg = err?.error?.error || 'Unable to place pre-booking reservation. Please try again.';
